            ...
            case ENET_EVENT_TYPE_RECEIVE: {
                if (event.packet->data) {
                    ...
                    case PACKET_SEND_INVENTORY_STATE: {
                        ...
                        inventory.resize(*reinterpret_cast<short*>(extended_ptr + 10));
                        memcpy(inventory.data(), extended_ptr + 10, server::inventory.capacity() * sizeof(Item));
...
void server::request() {
    ...
    HINTERNET hData = HttpOpenRequestA(hConnection, "POST", "/growtopia/server_data.php", NULL, NULL, NULL, INTERNET_FLAG_KEEP_CONNECTION, 0);
